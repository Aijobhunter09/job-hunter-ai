...
export function JobDetailsPage({ jobId }: JobDetailsPageProps) {
  ...
  const job: Job | undefined = demoJobs.find((j) => j.id === jobId);

  if (!job) {
    return (
      <div className="mx-auto max-w-3xl px-4 py-16 text-center">
        <h1 className="text-2xl font-bold text-slate-900">Job not found</h1>
        <p className="mt-2 text-slate-600">This job may have been removed or doesn't exist.</p>
        <div className="mt-6">
  ...
  const strengths = ['React', 'TypeScript', 'Git', 'API development'];
  const missing = ['AWS'];
    ...
    <div className="mx-auto max-w-5xl px-4 py-8 sm:px-6 lg:px-8">
      ...
      <div className="card p-6">
          ...
          <div className="flex items-start gap-4">
            ...
            <div>
              ...
              <div className="mt-3 flex flex-wrap items-center gap-x-4 gap-y-1.5 text-sm text-slate-500">
                ...
                <span className="flex items-center gap-1.5"><Calendar className="h-4 w-4" /> {job.postedDate}</span>
              </div>
            </div>
          </div>
        ...
        <div className="mt-5 flex flex-wrap gap-2">
          <Badge variant={job.workMode === 'Remote' ? 'success' : 'info'}>{job.workMode}</Badge>
          <Badge variant="neutral">{job.experienceLevel} level</Badge>
        ...
        <div className="mt-6 flex flex-col gap-3 sm:flex-row">
          <Button onClick={() => navigate('/applications')} className="flex-1 sm:flex-none">
        ...
        <div className="space-y-6">
          <div className="card p-6">
            <div className="flex items-center gap-2">
              ...
              <h2 className="text-lg font-semibold text-slate-900">AI Match Analysis</h2>
            </div>
            ...
            {!analyzed ? (
              <div className="mt-4">
                ...
                <Button onClick={() => setAnalyzed(true)} className="mt-4 w-full">
                  <Sparkles className="h-4 w-4" /> Analyze My Match
                </Button>
              </div>
            ) : (
              <div className="mt-4 animate-slide-up">
                <div className="rounded-lg bg-primary-50 p-4 text-center">
                  <p className="text-sm text-slate-600">Your Match</p>
                  <p className="text-3xl font-bold text-primary-700">{job.matchScore}%</p>
                </div>
                ...
                <div className="mt-4">
                  <h3 className="text-sm font-semibold text-amber-700">Missing / Preferred</h3>
                  <ul className="mt-2 space-y-1.5">
                    {missing.map((m) => (
                      <li key={m} className="flex items-center gap-2 text-sm text-slate-600">
                        <XCircle className="h-4 w-4 text-amber-500" /> {m}
                      </li>
                    ))}
                  </ul>
                </div>
                ...
                <p className="mt-4 rounded-lg bg-slate-50 p-3 text-xs text-slate-500">
                  Demo analysis based on sample profile data.
                </p>
              </div>
            )}
          </div>
